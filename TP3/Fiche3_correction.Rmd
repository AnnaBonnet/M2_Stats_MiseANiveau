...
### Question 1
```{r}
# a) échantillon de variables aléatoires de loi uniforme sur [0,1] :
...
# c) superposer la fonctin de répartition de la loi  uniforme sur [0,1]
u=seq(0,1,by=0.01)
y=punif(x)
lines(x,y,col=2)

## Avec ggplot
data_ecdf=data.frame(x=x,ecdf=fn(x),y=punif(x))

ggplot(data_ecdf,aes(x=x))+geom_step(aes(y=ecdf))+geom_line(aes(y=y),color="red")
...
La p-valeur est très élevée, il y a donc de grandes chances que l'échantillon simulé provienne bien d'une loi uniforme (heureusement !).
...
## Exercice 4 : Régression linaire simple (cas $p=1$)
...
## Exercice 5 : Régression linéaire multiple (cas $p>1$)
