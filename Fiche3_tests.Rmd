...
Si l'échantillon est de "grande taille" (souvent on considère en pratique que c'est le cas si $n \geq 30$), alors on considère l'approximation du TCL valable, i.e. 
 ...
 Enfin, si l'échantillon est de petite taille, on pourra considérer un test non paramétrique sur la variance (cf plus loin).
...
Dans le cas de petits échantillons gaussiens, on considérera plutôt un test non paramétrique décrit ci-dessous.
...
anova(res)
```
La commande **anova(res)** renvoie notamment les p-valeurs associées aux tests de Fisher.  
...
1. Simuler un échantillon $x$ de $n=20$ \vas de loi uniforme sur
  $[0,1]$.
